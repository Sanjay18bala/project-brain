...
| # | Slice | Depends on |
|---|---|---|
| 6 | Real event logging (`events` table) + sender/author identity capture | nothing — foundational |
| 7 | Semantic evidence search (embeddings + pgvector query), wired into `/agent/investigate` | 6 (more complete evidence to search) |
| 8 | Staleness/`UNKNOWN` detection + the periodic sweep worker | 6 |
| 9 | Deadline tracking (`DEADLINE` nodes, `DUE_BEFORE`, crossed-deadline detection) | 6, 8 (reuses the sweep) |
| 10 | Outbound Slack alerting (`actions` table, bot client, conflict/deadline/staleness alert policies) | 6, 8, 9 |
| 11 | PM Dashboard view (`PRD.md §10.1`) | 6–9 (needs the data to summarize) |

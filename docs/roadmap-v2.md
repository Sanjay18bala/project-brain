...
**Slice 10 follow-up, not yet built**: only conflict alerts exist. Deadline-crossed alerts and stale-work
nudges (both listed in "Expanded use cases" above) would reuse the same `actions`/`send_dm` machinery —
`run_staleness_sweep` and a deadline check would each need their own alert-sending step, mirroring
